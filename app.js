'use strict';
// Open through October 4, 2026, at 11:59 p.m., Guatemala time.
// End at midnight immediately afterward.
const PROMO_END = Date.parse('2026-10-05T00:00:00-06:00');
const ENROLLMENT_END = Date.parse('2026-10-05T00:00:00-06:00');
const plans = { Curso: { promo:49, regular:69, months:4 } };
// One course, two language-specific links. Prices are managed in PayPal.
const PAYPAL_CHECKOUT = { Curso: {
...
function checkoutFor(name, now=Date.now()) {
 if (!offerState(now).open) return null;
 return PAYPAL_CHECKOUT[name]?.[lang] || null;
}
let lang = 'es';
let selectedPlan = 'Curso';
...
function setLanguage(value){
 ...
 document.querySelector('meta[name="description"]').content=lang==='en'?'Create an interactive story with Ren’Py and Python fundamentals. An 8-week course with recorded lessons, weekly feedback.':'Aprende a crear una historia interactiva con Ren’Py y fundamentos de Python. Curso de 8 semanas, clases grabadas, revisión semanal.';
